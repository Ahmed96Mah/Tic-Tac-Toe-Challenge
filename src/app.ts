let turn: number[] = [1,2], plays: number = 0;

// Font Awesome icons Animations
const fontAwesomeAnimate = (evt: Event) => {
    if(((evt.target! as HTMLElement).nodeName === 'I') && (evt.target! as HTMLElement).classList.contains('fa-brands')) {
...
const processClick = (evt: Event) => {
    ...
    if (((evt.target! as HTMLElement).nodeName === 'I') && (evt.target! as HTMLElement).classList.contains('fa-circle')) {
        // First, Check that the game hasn't started yet.
        let emptyGrid: string;
        if ((evt.target! as HTMLElement).parentElement!.classList.contains('choice')) {
        ...
        if (emptyGrid! === '') { // If it is clear, then allow choice selection & modification.
            if ((evt.target! as HTMLElement).dataset.div === 'one') { 
                ...
                if (!(plyr1Choices[1].classList.contains('active')) && !(plyr2Choices[0].classList.contains('active'))) {
                    ...
                    setTimeout(() => {(evt.target! as HTMLElement).classList.toggle('fa-shake');}, 400);
                }
            }else {
                // make sure that only one symbol is selected in each choice div
    ...
    }else if (((evt.target! as HTMLElement).nodeName === 'I') && (evt.target! as HTMLElement).classList.contains('fa-xmark')) {
        // First, Check that the game hasn't started yet.
        let emptyGrid: string;
        if ((evt.target! as HTMLElement).parentElement!.classList.contains('choice')) {
            emptyGrid = checkGrid();
    ...
    }else if (((evt.target! as HTMLElement).nodeName === 'DIV') && (evt.target! as HTMLElement).classList.contains('content')) {
        ...
        if (!(choice1 === '') && !(choice2 === '')) { // make sure both players have provided their choices first
            if (!((evt.target! as HTMLElement).classList.contains('active'))) { // if it isn't an already activated cell
                if (turn[0] === 1) {
                    if (choice1 === 'o') {
                        (evt.target! as HTMLElement).innerHTML = '<i class="fa-regular fa-circle"></i>';
                        (evt.target! as HTMLElement).classList.toggle('o');
                        (evt.target! as HTMLElement).classList.toggle('active');
                    }else {
                        (evt.target! as HTMLElement).innerHTML = '<i class="fa-solid fa-xmark"></i>';
                ...
                (evt.target! as HTMLElement).querySelector('i')!.classList.toggle('fa-shake');
                setTimeout(() => {(evt.target! as HTMLElement).querySelector('i')!.classList.toggle('fa-shake');}, 400);
            }
        }else {
...
// Check Players' turns
const displayTurn = (plyr1Choice: NodeListOf<HTMLElement>, plyr2Choice: NodeListOf<HTMLElement>) => {
...
const processChoice = (plyr: number, plyr1Choice: NodeListOf<HTMLElement>, plyr2Choice: NodeListOf<HTMLElement>)=> {
...
const checkWin = (plyr1Choice: string, numOfPlays: number) => {
    ...
    for (const cell of row1) { // Check what occupies the cells on row 1
    ...
    for (const cell of row2) { // Check what occupies the cells on row 2
        if (cell.classList.contains('o')) { // If the cell has o
            Arr2.push('o');
        }else if (cell.classList.contains('x')) { // If the cell has x
            Arr2.push('x');
    ...
    for (const cell of row3) { // Check what occupies the cells on row 3
        if (cell.classList.contains('o')) { // If the cell has o
            Arr3.push('o');
        }else if (cell.classList.contains('x')) { // If the cell has x
            Arr3.push('x');
